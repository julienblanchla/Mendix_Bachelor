...
public class FileHandling {
	private static final ILogNode logger = Util.getLogger("FileHandling");
	...
	public static Boolean storeURLToFileDocument(IContext context, String url, IMendixObject __document, String filename) throws IOException {
		...
		try {
			...
			logger.trace(String.format("Remote filesize: %d", contentLength));
			...
			try (InputStream connectionInputStream = connection.getInputStream()) {
				if (contentLength >= 0) {
					...
					logger.trace(String.format("Unknown content length; limiting to %d", MAX_REMOTE_FILESIZE));
			...
			logger.trace(String.format("A problem occurred while reading from URL %s: %s", url, ioe.getMessage()));
			throw ioe;
		}
	...
	public static Long getFileSize(IContext context, IMendixObject document) {
		...
		if (context != null) {
			...
			try (
				...
				logger.error("Couldn't determine filesize of FileDocument '" + document.getId());
			}
		}
	...
	public static void setRespondWithFile(IContext context, IMxRuntimeResponse response, IMendixObject document) {
		final int BUFFER_SIZE = 4096;		
		if (context != null) {
			byte[] buffer = new byte[BUFFER_SIZE];
			try (
				...
				logger.error("Read file error '" + document.getId());
			}
		}
	}
	
	public static FileDocument saveFile(IContext ctx, InputStream stream, String name)
            throws CoreException, IOException {
        var docObj = new FileDocument(ctx);
